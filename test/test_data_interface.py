...
class Test_DataInterface(Test):
    ...
    def generate_script(self):
        ...
        # only receive low frequency package
        scripter.stop()
    ...
    def analyze(self, C, uart, commander, figures_dir):
        ...
        for packet in C.cont:
            if type(packet) == uc.Packet_Heartbeat:
                hb_num += 1
                # if last_hb is None:
                #     last_hb = packet.packet_count
                #     last_hbtime = packet.time
                #     hb_tmin = 1e9
                # else:
                #     if packet.packet_count != last_hb + 1 or packet.ok == False:
                #         self.results['result'] = 'FAILED'
                #         heartbeat_counter_ok = False
                #         passed = False
                #     else:
                #         last_hb = packet.packet_count
                #     dt = packet.time - last_hbtime
                #     last_hbtime = packet.time
